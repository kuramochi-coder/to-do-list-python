"""This module provides the To-Do-List CLI."""
# todolist/cli.py
...
from todolist import __app_name__, __version__, cli, ERRORS, config, database, todolist
...
def get_todoer() -> todolist.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "todolist init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return todolist.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "todolist init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
